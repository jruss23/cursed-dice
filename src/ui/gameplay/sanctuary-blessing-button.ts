...
export class SanctuaryBlessingButton extends BlessingButtonBase<SanctuaryBlessingButtonConfig> {
  ...
  protected create(): void {
    this.createButtonBase(PALETTE.gold[500], PALETTE.gold[800], PALETTE.gold[500], toDPR(-7));

    // Banked preview (shows small dice values when banking)
    this.bankedPreview = createText(this.scene, 0, toDPR(9), '', {
  ...
  protected updateDisplay(): void {
    ...
    if (canBank) {
      ...
      // Ready to restore - gold theme (consistent with blessing), clickable
      this.labelText.setText('RESTORE');
      this.labelText.setColor(COLORS.TEXT_WARNING);
      this.labelText.setY(toDPR(-7));
      ...
      this.buttonBg.setFillStyle(PALETTE.gold[900], ALPHA.BORDER_SOLID);
      this.buttonBg.setStrokeStyle(toDPR(2), PALETTE.gold[600], ALPHA.BORDER_MEDIUM);
